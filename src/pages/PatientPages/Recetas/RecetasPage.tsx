import { SdtRecetasPacAppItemProps, SdtrecetaspacappProps } from "./interface/recetas.interface";
...
export const RecetasPage = () => {
	...
	const [viewData, setViewData] = useState<SdtRecetasPacAppItemProps[] | null>(null);
	...
	useEffect(() => {
		if (getRecipes.response) {
			if (getRecipes.response?.Sdtrecetaspacapp !== null) {
				setData(getRecipes.response?.Sdtrecetaspacapp?.["sdtRecetasPacApp.sdtRecetasPacAppItem"]);
			} else {
				setData(null);
				dispatch(
					setToast({
						...
						Object(data).map((item: SdtrecetaspacappProps, index: number) => (
							<div key={index} className={style.card}>
								<div className={style.card__content}>
									...
									<label>
										Fecha: <span>{item && moment(item.Fecha).format("DD-MM-YYYY")}</span>
									</label>
								</div>
								<div style={{ paddingTop: "10px" }}>
									<Button
										...
										onClick={() =>
											onViewData(item.Productos["sdtRecetasPacApp.sdtRecetasPacAppItem.producto"])
										}
									>
